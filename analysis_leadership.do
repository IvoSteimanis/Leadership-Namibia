...
* Directory
*--------------------------------------------------
/* set your personal directories to load datasets and save figures
global workpath ""
global datapath ""
global output ""
*/
...
coefplot (c_1, aseq(Decision 1, n=64) ///
		...
		title("{bf:A:} democratic vs. autocratic", size(large))
graph save "$output\fig_1a", replace
...
coefplot (ols1, aseq(Decision 1, n=23) ///
		...
		mlabel format(%9.2f) mlabposition(11) mlabgap(*2)  ///
		legend(order(1 "95% CI" 2 "90% CI") pos(3) ring(0) size(medium) bmargin(tiny)) ///
		title("{bf:B:} pseudo-democratic vs. dictator", size(large))  xtitle("Estimated difference in means (DELs - chiefs)", size(large))
graph save "$output\fig_1b", replace

grc1leg fig_1a.gph fig_1b.gph, col(1) pos(2) ring(0)
graph combine "$output\fig_1a.gph" "$output\fig_1b.gph", scale(*1.2) col(1)
...
graph save "$output\fig_2a", replace
...
graph save "$output\fig_2b", replace
grc1leg "$output\fig_2a.gph" "$output\fig_2b.gph", pos(6) ring(1)
...
cibar social_types,  over1(id2) over2(st_id) barlabel(on) bargap(10) blsize(medium) blpos(11) /*
	...
	*/ ciopts(lpattern(dash) lcolor(gs0))  
	graph save "$output\fig_3a", replace
restore
...
cibar bigfive,  over1(leader) over2(big5_id) barlabel(on) bargap(10) blsize(medium) blpos(11) /*
	...
	*/  ciopts(lpattern(dash) lcolor(gs0))  
	graph save "$output\fig_3b", replace
restore

graph combine "$output\fig_3a.gph" "$output\fig_3b.gph",scale(*1)col(1)
...
cibar m_fg, over1(leader) /*
	...
	*/ ciopts(lpattern(dash) lcolor(gs0))  
graph save "$output\fig_6a", replace
...
graph save "$output\fig_6b", replace

grc1leg "$output\fig_6a.gph" "$output\fig_6b.gph", col(2) pos(6) ring(1)
...
cibar rel_trust, over1(regions_new) barlabel(on) blfmt(%9.2f) blsize(medium) blpos(6)  blgap(-0.05) /*
	...
	*/  ytitle("Mean", size(medium))) ciopts(lpattern(dash) lcolor(gs0))
	gr save "$output\relative_trust", replace
...
cibar rel_performance, over1(regions_new) barlabel(on) blfmt(%9.2f) blsize(medium) blpos(6)  blgap(-0.05) /*
	...
	*/  ytitle("Mean", size(medium))) ciopts(lpattern(dash) lcolor(gs0))   
	gr save "$output\relative_performance", replace
grc1leg "$output\relative_trust" "$output\relative_performance", pos(6) ring(1)
gr export "$output\figure5.emf", replace
